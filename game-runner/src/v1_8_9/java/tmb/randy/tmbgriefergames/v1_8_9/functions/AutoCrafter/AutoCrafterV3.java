...
import static tmb.randy.tmbgriefergames.v1_8_9.functions.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP1;
import static tmb.randy.tmbgriefergames.v1_8_9.functions.AutoCrafter.AutoCrafterV3.COMP_STATE.FINISHED;
import static tmb.randy.tmbgriefergames.v1_8_9.functions.AutoCrafter.AutoCrafterV3.COMP_STATE.IDLE;
import static tmb.randy.tmbgriefergames.v1_8_9.functions.AutoCrafter.AutoCrafterV3.COMP_STATE.OPEN_COMP;
...
public class AutoCrafterV3 extends ActiveFunction {
    ...
    public void tickEvent(GameTickEvent event) {
        if(isEnabled()) {
            ...
            if(craftItem == null) {
                if(Helper.getPlayer().inventory.mainInventory[0] != null) {
                    if(Helper.getPlayer().openContainer instanceof ContainerChest chest) {
                        if(ClickManager.getSharedInstance().isClickQueueEmpty(QueueType.MEDIUM)) {
                            ...
                            if(chestInventory.getName().equals("§6Custom-Kategorien")) {
                                ...
                                if(Helper.getPlayer().inventory.mainInventory[0].getItem().equals(Items.gold_ingot)) {
                            ...
                            } else if(chestInventory.getName().equals("§6Vanilla Bauanleitung") && areItemStacksEqual(chestInventory.getStackInSlot(25), Helper.getPlayer().inventory.mainInventory[0])) {
                        ...
                        if(lookingAtBlock != null) {
                            if(lookingAtBlock.equals(neededBlock)) {
                                if(!Helper.getPlayer().isSneaking()) {
                                    MovingObjectPosition trace = Helper.getPlayer().rayTrace(5, 1.0F);
                                    if(trace != null)
                                        Minecraft.getMinecraft().playerController.onPlayerRightClick(Helper.getPlayer(), Helper.getWorld(), Helper.getPlayer().getHeldItem(), lookingAtBlock, trace.sideHit, trace.hitVec);
                                }
                            } else {
                ...
                switch (chestInventory.getName()) {
                    ...
                    case "§6Vanilla Bauanleitung": {
                        ...
                        if(nextItem != null)
                            ...
                            if(variantPage != -1) {
                                if(chestInventory.getStackInSlot(49) != null && chestInventory.getStackInSlot(49).stackSize > 0) {
    ...
    private int getFirstSlotForCraftItem() {
        for (int i = 0; i < Helper.getPlayer().inventory.mainInventory.length; i++) {
            ItemStack itemStack = Helper.getPlayer().inventory.mainInventory[i];
    ...
    private void click(int slot) {
        ClickManager.getSharedInstance().addClick(
            QueueType.MEDIUM,
            new Click(Helper.getPlayer().openContainer.windowId, slot, 0, 1)
        );
    }
