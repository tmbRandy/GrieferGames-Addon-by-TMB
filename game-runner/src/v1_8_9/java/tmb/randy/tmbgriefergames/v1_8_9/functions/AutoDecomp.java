...
public class AutoDecomp extends ActiveFunction {
    ...
    private void decomp() {
        if (Helper.getPlayer().openContainer != null
            ...
            if (ClickManager.getSharedInstance().isClickQueueEmpty(QueueType.MEDIUM)) {
                if (compItem == null) {
                    if (Helper.getPlayer().inventory.mainInventory[0] != null && Helper.getPlayer().inventory.mainInventory[0].stackSize > 0) {
                        compItem = Helper.getPlayer().inventory.mainInventory[0].getItem();
                        compSubID = Helper.getPlayer().inventory.mainInventory[0].getMetadata();
                    }
                } else {
                    //Do Decomp
    ...
    private boolean doesCraftingFieldContainItems() {
        if (Helper.getPlayer().openContainer instanceof ContainerWorkbench) {
            for (int i = 1; i < 10; i++) {
                if (Helper.getPlayer().openContainer.getSlot(i).getHasStack()) {
                    return true;
                }
            }
        }
    ...
    private void dropUncompressedItems() {
        if (Helper.getPlayer().openContainer instanceof ContainerWorkbench) {
            for (int i = 10; i < 46; i++) {
                if (Helper.getPlayer().openContainer.getSlot(i).getHasStack()) {
                    if (Helper.getPlayer().openContainer.getSlot(i).getStack().getItem().equals(compItem) && Helper.getPlayer().openContainer.getSlot(i).getStack().getMetadata() == compSubID) {
                        if (!Helper.getPlayer().openContainer.getSlot(i).getStack().hasDisplayName()) {
    ...
    private int getBestSlotForCompStep(int step) {
        ...
        if (Helper.getPlayer().openContainer instanceof ContainerWorkbench) {
            for (int i = 10; i < Helper.getPlayer().openContainer.inventorySlots.size(); i++) {
                if (Helper.getPlayer().openContainer.inventorySlots.get(i)
                    .getHasStack()) {
                    if (Helper.getPlayer().openContainer.inventorySlots.get(i).getStack().getItem().equals(compItem) && Helper.getPlayer().openContainer.inventorySlots.get(i).getStack().getMetadata() == compSubID) {
                        if (Helper.getPlayer().openContainer.inventorySlots.get(i)
                            .getStack().hasDisplayName()) {
                            if (Helper.getPlayer().openContainer.inventorySlots.get(i)
                                .getStack().getDisplayName().endsWith(" " + romanian)) {
                                if (lowestItemStack == null) {
                                    lowestItemStack = Helper.getPlayer().openContainer.inventorySlots.get(
                                        i).getStack();
                                    lowestSlot = i;
                                } else if (lowestItemStack.stackSize
                                    > Helper.getPlayer().openContainer.inventorySlots.get(
