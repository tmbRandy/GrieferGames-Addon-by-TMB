...
public class AutoCrafterV2 extends ActiveFunction {
    ...
    public boolean start() {
        if(super.start()) {
            if(Helper.getPlayer().inventory.getStackInSlot(0) != null) {
    ...
    private int getSlotCountOfItemInInventory() {
        ...
        for (ItemStack itemStack : Helper.getPlayer().inventory.mainInventory) {
            if(itemStack != null && itemStack.getItem().equals(itemToCraft) && itemStack.getMetadata() == subIDtoCraft) {
                count++;
            }
        }
        return count;
    ...
    private int getSlotForItemToCraft() {
        ...
        for (int i = 54; i < slotCount; i++) {
            ...
            if(stack != null && stack.getItem().equals(itemToCraft) && stack.getMetadata() == subIDtoCraft) {
                return i;
            }
        }
        return -1;
