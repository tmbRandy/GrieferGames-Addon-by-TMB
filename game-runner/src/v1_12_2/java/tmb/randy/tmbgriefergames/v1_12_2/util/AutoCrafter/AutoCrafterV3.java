...
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP1;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP2;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP3;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP4;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP5;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.COMP6;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.FINISHED;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.IDLE;
import static tmb.randy.tmbgriefergames.v1_12_2.util.AutoCrafter.AutoCrafterV3.COMP_STATE.OPEN_COMP;
...
public class AutoCrafterV3 {
    ...
    public void onTick() {
        if(active) {
            ...
            StuckProtection.tick(Minecraft.getMinecraft().player.openContainer);
                    ...
                    if(Minecraft.getMinecraft().player.openContainer instanceof ContainerChest chest) {
                        if(ClickManager.getSharedInstance().isClickQueueEmpty(QueueType.MEDIUM)) {
                            ...
                            if(chestInventory.getName().equals("§6Custom-Kategorien")) {
                                ...
                                if(Minecraft.getMinecraft().player.inventory.mainInventory.getFirst().getItem().equals(Items.GOLD_INGOT)) {
                            ...
                            } else if(chestInventory.getName().equals("§6Vanilla Bauanleitung") && areItemStacksEqual(chestInventory.getStackInSlot(25), Minecraft.getMinecraft().player.inventory.mainInventory.getFirst())) {
                ...
                if(allSourceChestsScanned()) {
                    ...
                    if(Minecraft.getMinecraft().player.openContainer instanceof ContainerChest chest) {
                        ...
                        if(chestItemStack != null) {
                            ...
                            RayTraceResult trace = Minecraft.getMinecraft().player.rayTrace(5, 1.0F);
    ...
    private void craft() {
        ...
        if(numberOfFinishedStacks <= 1 || (Addon.getSharedInstance().configuration().getAutoCrafterConfig().getFinalActionV3().get() == AutoCrafterNewFinalAction.COMP && compState == FINISHED)) {
                        ...
                        if(lookingAtBlock != null) {
                            if(lookingAtBlock.equals(neededBlock)) {
                                if(!Minecraft.getMinecraft().player.isSneaking()) {
                                    RayTraceResult trace = Minecraft.getMinecraft().player.rayTrace(5, 1.0F);
                                    if(trace != null) {
                                        Minecraft.getMinecraft().playerController.processRightClickBlock(Minecraft.getMinecraft().player, Minecraft.getMinecraft().world, lookingAtBlock, trace.sideHit, trace.hitVec, EnumHand.MAIN_HAND);
                                    }
                                }
                            } else {
            ...
            } else if(Minecraft.getMinecraft().player.openContainer instanceof ContainerChest chest) {
                IInventory chestInventory = chest.getLowerChestInventory();
            ...
            if(Addon.getSharedInstance().configuration().getAutoCrafterConfig().getFinalActionV3().get() == AutoCrafterNewFinalAction.DROP) {
                ...
                if(Minecraft.getMinecraft().player.openContainer instanceof ContainerChest) {
                    closeChest();
                } else if(Minecraft.getMinecraft().player.openContainer instanceof ContainerPlayer inv && Minecraft.getMinecraft().currentScreen instanceof GuiInventory) {
    ...
    public static boolean isContainerOpen() {
        ...
        return openContainer != mc.player.inventoryContainer;
    }
    ...
    private void lookAtBlockPos(BlockPos pos) {
        Vec3d playerPos = Minecraft.getMinecraft().player.getPositionEyes(1.0F);
    ...
    private void comp() {
        switch (compState) {
            case OPEN_COMP -> {
                if(Minecraft.getMinecraft().player.openContainer instanceof ContainerChest chest) {
                    ...
                    if(inv.getName().equalsIgnoreCase("§6Custom-Kategorien")) {
                        ClickManager.getSharedInstance().addClick(QueueType.MEDIUM, new Click(Minecraft.getMinecraft().player.openContainer.windowId, 11, 0, ClickType.QUICK_MOVE));
                    } else if(inv.getName().equalsIgnoreCase("§6Item-Komprimierung-Bauanleitung")) {
                        int clickSlot = getBestCompSlot();
                        ClickManager.getSharedInstance().addClick(QueueType.MEDIUM, new Click(Minecraft.getMinecraft().player.openContainer.windowId, clickSlot, 0, ClickType.QUICK_MOVE));
            ...
            case COMP1, COMP2, COMP3, COMP4, COMP5, COMP6, FINISHED -> {
                if(ClickManager.getSharedInstance().isClickQueueEmpty(QueueType.MEDIUM)) {
                    if(Minecraft.getMinecraft().player.openContainer instanceof ContainerChest && Minecraft.getMinecraft().player.openContainer.getSlot(49).getStack() != null) {
    ...
    private void compClick() {
        ClickManager.getSharedInstance().addClick(QueueType.MEDIUM, new Click(Minecraft.getMinecraft().player.openContainer.windowId, 52, 0, ClickType.QUICK_MOVE));
    }

    private void increaseStep() {
        ClickManager.getSharedInstance().addClick(QueueType.MEDIUM, new Click(Minecraft.getMinecraft().player.openContainer.windowId, 50, 0, ClickType.QUICK_MOVE));
    }

    private void decreaseStep() {
        ClickManager.getSharedInstance().addClick(QueueType.MEDIUM, new Click(Minecraft.getMinecraft().player.openContainer.windowId, 48, 0, ClickType.QUICK_MOVE));
    }
    ...
    private int getBestCompSlot() {
        ...
        for (int i = 54; i < 90; i++) {
            Slot slot = Minecraft.getMinecraft().player.openContainer.inventorySlots.get(i);
            if(slot.getHasStack()) {
